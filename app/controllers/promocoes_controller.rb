class PromocoesController < ApplicationController
  ...
  def load_dynamic
   @DynamicPage = DynamicPage.first(:conditions => ['id = ? AND published = 1', ID_PROMOCOES])
    ...
    @Header = Header.first(:conditions => ['dynamic_page_id = ? AND published = 1', ID_PROMOCOES], :order => 'id DESC')
 end
end
